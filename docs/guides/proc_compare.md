...
如果 `compare_cols` 中某些字段只存在于一侧，不会直接报错；它们会出现在 `schema_summary` 中，状态为 `left_only_column` 或 `right_only_column`。只有两边都存在的字段才进入字段值比较。
...
| `schema_summary` | 每个字段在左右表是否存在、dtype、字段角色、schema 状态。 |
| `column_summary` | 每个比较字段的 `n_compared`、`n_mismatch`、缺失数、差异统计。 |
