...
| 子项目 | 功能定位 | 核心能力 |
|--------|---------|---------|
| **[Modeling_Tool](https://example.com/SuperModelingFactory)** | 建模引擎 | 数据分箱、WOE 编码、特征分析、模型训练与评估、样本管理 |
| **[ExcelMaster](https://example.com/SuperModelingFactory)** | 报告引擎 | 程序化 Excel 工作簿生成，支持图表、条件格式、光标流式写入 |
| **[Report](https://example.com/SuperModelingFactory)** | 报告模板 | 模型性能报告、WOE 图批量导出、多模型对比报告 |
...
!!! tip "典型场景"
    ...
    - 训练 **逻辑回归 / LightGBM / XGBoost** 模型并自动出 Gains / ROC / KS 报告
    - 处理**拒绝推断**与**分布偏移**问题
